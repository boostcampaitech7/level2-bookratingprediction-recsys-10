...
def main(args):
    ...
    # loss_fn = MSELoss()
    y = pd.read_csv(args.result_path + "y_vaild.csv")
    # total_loss = 0

    # # y와 result를 텐서로 변환
    # y_tensor = torch.tensor(y['rating'].values, dtype=torch.float32)
    # result_tensor = torch.tensor(result, dtype=torch.float32)

    # # 전체 손실 계산
    # loss = loss_fn(y_tensor, result_tensor)
    # total_loss = loss.item()

    mse = mean_squared_error(y['rating'], output['rating'])
    total_loss = np.sqrt(mse)
    
    print(f"RMSE LOSS : {total_loss}")
...
if __name__ == "__main__":
    ...
    arg('--result_path',type=str, default='tem/',
