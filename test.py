...
from src.train import train, test, sub
...
def main(args, wandb=None):
    ...
    if not args.predict:
        ...
        predicts = sub(args, model, data, setting)
    else:
        print(f'--------------- {args.model} PREDICT ---------------')
        predicts = sub(args, model, data, setting, args.checkpoint)

    vaild = pd.read_csv(args.dataset.valid_path + 'y_vaild.csv')
    vaild['rating'] = predicts



    filename = setting.get_valid_filename(args)
    print(f'Save vaild: {filename}')
    vaild.to_csv(filename, index=False)
...
if __name__ == "__main__":
    ...
    arg('--model', '-m', '--m', type=str, 
        choices=['FM', 'FFM', 'DeepFM', 'NCF', 'WDN', 'DCN', 'Image_FM', 'Image_DeepFM', 'Text_FM', 'Text_DeepFM'
                 , 'ResNet_DeepFM', 'LightGCN', 'LightGBM','CatBoost','XGBoost','CatBoostClass'],
    ...
    arg('-v', '--version', nargs=1, type=str, help='파일 이름에 사용할 버전 이름 (예: m1)')
